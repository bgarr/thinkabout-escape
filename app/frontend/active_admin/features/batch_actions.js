...
const batchActionClick = function (event) {
  ...
  if (!event.target.dataset.confirm && !event.target.dataset.modalTarget) {
    submitForm();
  }
};
...
Rails.delegate(
  ...
  "confirm:complete",
  batchActionConfirmComplete
);
Rails.delegate(document, "[data-batch-action-item]", "click", batchActionClick);
Rails.delegate(
  document,
  "form[data-batch-action-form]",
  "submit",
  batchActionFormSubmit
);
...
const toggleAllChange = function (event) {
  const checkboxes = document.querySelectorAll(
    ".batch-actions-resource-selection"
  );
...
const toggleCheckboxChange = function (event) {
  const numChecked = document.querySelectorAll(
    ".batch-actions-resource-selection:checked"
  ).length;
  const allChecked =
    numChecked ===
    document.querySelectorAll(".batch-actions-resource-selection").length;
  const someChecked =
    numChecked > 0 &&
    numChecked <
      document.querySelectorAll(".batch-actions-resource-selection").length;
...
const tableRowClick = function (event) {
  ...
  if (
    ...
    (type !== "checkbox" && type !== "button" && type !== "")
  ) {
